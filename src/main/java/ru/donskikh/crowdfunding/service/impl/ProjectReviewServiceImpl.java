...
import ru.donskikh.crowdfunding.domain.repository.ProjectReviewRepository;
...
public class ProjectReviewServiceImpl implements ProjectReviewService {
    ...
    public ProjectReviewEntity create(UUID userId, UUID projectId, ProjectReviewRequest req) {
        ...
        if (project.getAuthor() != null && userId.equals(project.getAuthor().getId())) {
            throw new IllegalStateException("Project authors cannot post reviews for their own projects");
        }
